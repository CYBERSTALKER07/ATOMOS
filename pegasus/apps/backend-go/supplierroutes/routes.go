...
func RegisterRoutes(r chi.Router, d Deps) {
	...
	r.HandleFunc("/v1/supplier/billing/setup",
		auth.RequireRole(supplierRole, log(withRegionScope(idem(supplier.HandleBillingSetup(d.Spanner))))))
	...
	r.HandleFunc("/v1/supplier/gateway-onboarding",
		auth.RequireRole(supplierRole, log(withRegionScope(withMethodIdempotency(vault.HandleGatewayOnboarding(d.Spanner), idem, http.MethodPost, http.MethodDelete)))))
	r.HandleFunc("/v1/supplier/payment/recipient/register",
		auth.RequireRole(supplierRole, log(withRegionScope(idem(vault.HandleRegisterRecipient(d.Spanner, d.DirectClient))))))
	r.HandleFunc("/v1/supplier/org/members",
		auth.RequireRole(supplierRole, log(withRegionScope(withWarehouseScope(supplier.HandleOrgMembers(d.Spanner))))))
	r.HandleFunc("/v1/supplier/org/members/invite",
		auth.RequireRole(supplierRole, log(withRegionScope(idem(supplier.HandleOrgInvite(d.Spanner))))))
	r.HandleFunc("/v1/supplier/org/members/*",
		auth.RequireRole(supplierRole, log(withRegionScope(idem(supplier.HandleOrgMemberAction(d.Spanner))))))
	r.HandleFunc("/v1/supplier/staff/payloader",
		auth.RequireRole(supplierRole, log(withRegionScope(withMethodIdempotency(withWarehouseScope(supplier.HandleStaffPayloaders(d.Spanner)), idem, http.MethodPost)))))
	r.HandleFunc("/v1/supplier/staff/payloader/*",
		auth.RequireRole(supplierRole, log(withRegionScope(withMethodIdempotency(withWarehouseScope(supplier.HandlePayloaderDetail(d.Spanner)), idem, http.MethodPost)))))
	r.HandleFunc("/v1/supplier/warehouse-staff",
		auth.RequireRole(supplierRole, log(withRegionScope(withWarehouseScope(supplier.HandleWarehouseStaff(d.Spanner))))))
	r.HandleFunc("/v1/supplier/warehouse-staff/*",
		auth.RequireRole(supplierRole, log(withRegionScope(idem(withWarehouseScope(supplier.HandleWarehouseStaffToggle(d.Spanner)))))))
	r.HandleFunc("/v1/supplier/warehouses",
		auth.RequireRole(supplierRole, log(withRegionScope(idem(withWarehouseScope(supplier.HandleWarehouses(d.Spanner, d.Producer)))))))
	r.HandleFunc("/v1/supplier/warehouses/*",
		auth.RequireRole(supplierRole, log(withRegionScope(idem(withWarehouseScope(supplier.HandleWarehouseByID(d.Spanner, d.Producer)))))))
	r.HandleFunc("/v1/supplier/warehouse-inflight-vu",
		auth.RequireRole(supplierRole, log(withRegionScope(withWarehouseScope(supplier.HandleWarehouseInflightVU(d.Spanner))))))

	if d.SupplierHub != nil {
		r.HandleFunc("/v1/ws/supplier",
			auth.RequireRole(supplierRole, withRegionScope(d.SupplierHub.HandleConnection)))
	}
}
