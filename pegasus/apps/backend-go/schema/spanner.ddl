...
-- ── SUPPLIER PAYOUT POLICY SNAPSHOTS ─────────────────────────────────────
...
-- ── SUPPLIER IMPORT SANDBOX (PHASE 2 COMPATIBILITY SUBSTRATE) ─────────────
-- Additive supplier-scoped import substrate that keeps staged data isolated
-- by supplier_id and session_id for deterministic verification workflows.
...
CREATE INDEX Idx_Refunds_Pending ON Refunds(Status) WHERE Status IS NOT NULL;
...
CREATE INDEX Idx_DispatchLocks_Active
    ON DispatchLocks(SupplierId, UnlockedAt, LockedAt DESC);
...
-- Keep unresolved-exception lookups emulator-safe without inverting NULL semantics.
CREATE INDEX Idx_ManifestExceptions_Unresolved ON ManifestExceptions(SupplierId, ResolvedAt, CreatedAt DESC);
...
-- Keep pending dispatch scans emulator-safe without filtered-index predicates.
CREATE INDEX Idx_Orders_ByDispatchPriority ON Orders(SupplierId, State, DispatchPriority DESC, CreatedAt ASC);
...
-- Already added in the warehouse-linkage tranche above for fresh canonical boot.
...
-- AssignedFactoryId already exists in the canonical CREATE TABLE above.
...
-- Keep unresolved-audit lookups emulator-safe without inverting NULL semantics.
CREATE INDEX Idx_DispatchAudit_Unresolved ON DispatchAudit(SupplierId, ResolvedAt, CreatedAt DESC);
...
-- UnavailableReason already exists in the canonical CREATE TABLE above.
...
-- have not yet been upgraded to send a trace_id.
...
CREATE INDEX Idx_DriverTelemetry_ByDriverTime
    ON DriverTelemetry(DriverId, EventTime DESC);

CREATE INDEX Idx_DriverTelemetry_BySupplierTime
    ON DriverTelemetry(SupplierId, EventTime DESC);
...
CREATE INDEX Idx_Orders_ByAutoAcceptEligible
    ON Orders(State, CancelLockedAt, RequestedDeliveryDate);
