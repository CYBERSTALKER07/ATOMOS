...
func (h *WarehouseHub) HandleConnection(w http.ResponseWriter, r *http.Request) {
	...
	h.mu.RLock()
	active := len(h.clients[warehouseID])
	h.mu.RUnlock()
	if !EnforceWSConnectionLimits(w, r, "warehouse", warehouseID, active) {
		return
	}
...
func (h *WarehouseHub) pushToWarehouseLocal(warehouseID string, data []byte) bool {
	...
	return writeGuardedPayload("warehouse", "warehouse_id", warehouseID, snapshot, &h.writeMu, data)
}
