...
func HandleBillingSetup(spannerClient *spanner.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		validGateways := map[string]bool{"GLOBAL_PAY": true, "CASH": true, "CARD": true, "BANK": true}
		if !validGateways[req.PaymentGateway] {
			http.Error(w, `{"error":"invalid payment_gateway — must be GLOBAL_PAY, CASH, CARD, or BANK"}`, http.StatusBadRequest)
			return
		}
