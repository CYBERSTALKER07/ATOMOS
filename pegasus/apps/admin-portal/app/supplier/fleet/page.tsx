...
export default function FleetPage() {
  ...
  const [formType, setFormType] = useState<"IN_HOUSE" | "CONTRACTOR">(
    "IN_HOUSE",
  );
      ...
      const capacityResponses = await Promise.all(
        routeIds.map(async (routeId) => {
          const res = await apiFetch(
            `/v1/fleet/capacity?route_id=${encodeURIComponent(routeId)}`,
          );
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const res = await apiFetch("/v1/supplier/fleet/drivers", {
        ...
        headers: {
          "Idempotency-Key":
            buildSupplierFleetDriverCreateIdempotencyKey(payload),
        },
      ...
      if (body.queued) {
        ...
        resetForm();
        toast(
          "Driver creation queued — PIN will be available after reconnect",
          "success",
        );
        return;
      }
    ...
    try {
      const res = await apiFetch(
        `/v1/supplier/fleet/drivers/${driverId}/assign-vehicle`,
        {
          method: "PATCH",
          headers: {
            "Idempotency-Key": buildSupplierFleetDriverAssignIdempotencyKey(
              driverId,
              vehicleId,
            ),
          },
      ...
      if (body.queued) {
        toast(
          "Vehicle assignment queued — will replay when back online",
          "success",
        );
        return;
      }
  ...
  async function handleCreateVehicle() {
    ...
    try {
      const res = await apiFetch("/v1/supplier/fleet/vehicles", {
        ...
        headers: {
          "Idempotency-Key":
            buildSupplierFleetVehicleCreateIdempotencyKey(body),
        },
      ...
      if (responseBody.queued) {
        ...
        setVehHeightCM("");
        toast(
          "Vehicle creation queued — will replay when back online",
          "success",
        );
        return;
      }
  ...
  async function handleDeactivateVehicle(vehicleId: string) {
    try {
      const res = await apiFetch(`/v1/supplier/fleet/vehicles/${vehicleId}`, {
        ...
        headers: {
          "Idempotency-Key":
            buildSupplierFleetVehicleDeactivateIdempotencyKey(vehicleId),
        },
      });
      const body = await res
        .json()
      ...
      if (body.queued) {
        toast(
          "Vehicle deactivation queued — will replay when back online",
          "success",
        );
        return;
      }
  ...
  async function handleClearReturns(vehicleId: string) {
    try {
      const res = await apiFetch(`/v1/vehicle/${vehicleId}/clear-returns`, {
        ...
        headers: {
          "Idempotency-Key":
            buildSupplierFleetClearReturnsIdempotencyKey(vehicleId),
        },
      });
      const body = await res
        .json()
      ...
      if (body.queued) {
        toast(
          "Return clearance queued — will replay when back online",
          "success",
        );
        return;
      }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1
            className="md-typescale-headline-medium mt-1"
        ...
        {(["drivers", "vehicles"] as FleetTab[]).map((tab) => (
          <button
            ...
            style={{
              ...
              color:
                activeTab === tab ? "var(--accent-foreground)" : "var(--muted)",
            }}
          >
            {tab === "drivers"
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          >
            ...
            <p
              ...
              style={{ color: "var(--accent-soft)" }}
            >
              {vehicles.reduce((sum, v) => sum + v.max_volume_vu, 0).toFixed(0)}{" "}
              VU
            </p>
          </div>
          <div
          ...
          >
            ...
            >
              {vehicles.filter((v) => v.assigned_driver_id).length} /{" "}
              {vehicles.length}
            </p>
          </div>
        </div>
      ...
      {activeTab === "vehicles" && (
        <>
          {vehicles.length === 0 ? (
            ...
            <div
              className="md-card-outlined overflow-hidden"
                ...
                <tbody>
                  {vehiclePagination.pageItems.map((v) => (
                    <tr key={v.vehicle_id}>
                      <td>
                        <span
                          ...
                          style={{
                            borderRadius: "99px",
                            background:
                              v.vehicle_class === "CLASS_C"
                                ? "color-mix(in srgb, var(--accent) 15%, transparent)"
                                : v.vehicle_class === "CLASS_B"
                                  ? "color-mix(in srgb, var(--accent-soft) 15%, transparent)"
                        ...
                        <span
                          className="md-typescale-body-small ml-2"
                          style={{ color: "var(--muted)" }}
                        >
                          {v.class_label}
                        </span>
                      ...
                      <td
                        className="md-typescale-body-medium font-mono"
                        style={{ color: "var(--muted)" }}
                      >
                        {v.license_plate || "—"}
                      </td>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          {v.is_active && (
                            <Button
                              ...
                              onPress={() =>
                                handleDeactivateVehicle(v.vehicle_id)
                              }
                            >
                              Deactivate
                            </Button>
                            ...
                            (() => {
                              ...
                              return vuPending > 0 ? (
                                <Button
                                  ...
                                  onPress={() =>
                                    handleClearReturns(v.vehicle_id)
                                  }
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              <p
                ...
                style={{ color: "var(--muted)" }}
              >
                No drivers provisioned yet
              </p>
              <p
                className="md-typescale-body-medium mt-2"
                style={{ color: "var(--muted)" }}
              >
                Click &quot;+ Add Driver&quot; to provision your first fleet
                member.
              </p>
            </div>
          ) : (
                    ...
                    >
                      ...
                      <td onClick={(e) => e.stopPropagation()}>
                        <select
                          value={d.vehicle_id || ""}
                          onChange={(e) =>
                            handleAssignVehicle(d.driver_id, e.target.value)
                          }
                          ...
                          {vehicles
                            ...
                            .map((v) => (
                              <option key={v.vehicle_id} value={v.vehicle_id}>
                                {v.class_label} ·{" "}
                                {v.license_plate || v.vehicle_id.slice(0, 8)}
                              </option>
                            ))}
                        </select>
                      </td>
                        ...
                        {!d.is_active && d.offline_reason && (
                          <span
                            ...
                            style={{
                              borderRadius: "99px",
                              background:
                                d.offline_reason === "TRUCK_DAMAGED"
            ...
            <div className="flex gap-2 mt-1">
              {(["IN_HOUSE", "CONTRACTOR"] as const).map((t) => (
                <button
                  ...
                  className={
                    formType === t ? "md-chip md-chip-selected" : "md-chip"
                  }
                  style={{
                    borderColor:
                      formType === t
                    ...
                    background:
                      formType === t
                        ? t === "IN_HOUSE"
                          ? "color-mix(in srgb, var(--accent-soft) 12%, transparent)"
            ...
            >
              ...
              {vehicles
                ...
                .map((v) => (
                  <option key={v.vehicle_id} value={v.vehicle_id}>
                    {v.vehicle_class.replace("CLASS_", "")} ·{" "}
                    {v.label || v.class_label} · {v.license_plate || "—"} (
                    {v.max_volume_vu} VU)
                  </option>
                ))}
            </select>
      ...
      {createdPin && (
        ...
        >
          ...
          >
            ...
            <h3
              className="md-typescale-title-large text-center"
              style={{ color: "var(--foreground)" }}
            >
              Driver Provisioned
            </h3>
            ...
            <p
              className="md-typescale-body-medium text-center"
              style={{ color: "var(--muted)" }}
            >
            ...
            >
              <p
                className="md-typescale-label-medium text-center"
                style={{ color: "var(--warning)" }}
              >
                <Icon
                  name="warning"
                ...
                />{" "}
                Copy this PIN. It will not be shown again.
              </p>
            </div>
          ...
          <div className="p-6 flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <DetailCell
                ...
                value={String(selectedDriver.total_deliveries)}
              />
              <DetailCell
                label="Status"
                value={selectedDriver.is_active ? "Active" : "Inactive"}
              />
              <DetailCell
                label="Provisioned"
                value={new Date(selectedDriver.created_at).toLocaleDateString()}
              />
              <DetailCell
                label="Driver ID"
        ...
        <div className="p-6 flex flex-col gap-4">
          <div>
            <div className="flex items-center justify-between">
              ...
              <div
                className="flex rounded overflow-hidden text-[10px] font-medium"
                style={{ border: "1px solid var(--border)" }}
              >
                <button
                  type="button"
                  onClick={() => setVehVuMode("class")}
                ...
                <button
                  type="button"
                  onClick={() => setVehVuMode("dimensions")}
                  className="px-3 py-1 transition-colors"
                  style={
                    vehVuMode === "dimensions"
                      ? {
            ...
            {vehVuMode === "class" ? (
              <p
                className="md-typescale-body-small mt-1"
                style={{ color: "var(--muted)" }}
              >
                Capacity:{" "}
                <strong>
                  {VEHICLE_CLASSES.find((c) => c.value === vehClass)?.vu ?? 0}{" "}
                  VU
                </strong>
              </p>
            ) : (
              <div className="mt-2 space-y-2">
                <div className="grid grid-cols-3 gap-2">
                  {(
                    [
                      ["L", vehLengthCM, setVehLengthCM],
                      ["W", vehWidthCM, setVehWidthCM],
                      ["H", vehHeightCM, setVehHeightCM],
                    ] as [
                      string,
                      string,
                ...
                {(() => {
                  const l = parseFloat(vehLengthCM),
                    w = parseFloat(vehWidthCM),
                    h = parseFloat(vehHeightCM);
                  const vu =
                    l > 0 && w > 0 && h > 0
                      ? ((l * w * h) / 5000).toFixed(2)
                      : null;
                  return (
                    <div
                    ...
                    >
                      ...
                      <span
                        className="md-typescale-label-large font-semibold"
                        style={{ color: "var(--accent-soft-foreground)" }}
                      >
                        {vu ? `${vu} VU` : "—"}
                      </span>
