...
- OR-Tools sidecar optimization runtime (Pattern-A): `pegasus/services/optimizer-core/**`
	...
	- Local no-cloud simulation is now first-class via Docker: `docker-compose.yml` now bootstraps `pegasus-optimizer-jobs` in `kafka-init` and adds profile `optimizer-sim` services (`optimizer-core-rust`, `optimizer-adapter-worker`), with orchestration targets `optimizer-sim-up`, `optimizer-sim-down`, and `optimizer-sim-bootstrap` in `Makefile`.
