...
- OR-Tools sidecar optimization runtime (Pattern-A): `pegasus/services/optimizer-core/**`
	- Rust sidecar is now the canonical production solver; Python sidecar under `services/optimizer-core/server/**` remains a deprecated reference-only parity path.
	- Protobuf contracts in `services/optimizer-core/proto/optimizer_core.proto` now add additive `SolverStatus` (`OPTIMAL`, `FEASIBLE`, `INFEASIBLE`, `MODEL_INVALID`) plus `matrix_size` on both VRP and CP-SAT response envelopes while retaining legacy `feasible` compatibility for mixed-rollout consumers.
	...
	- Go worker in `services/optimizer-core/adapters/go/**` now consumes shared `packages/optimizer-contract/OptimizationJobEnvelope` Kafka payloads, applies bounded retry/backoff+jitter around solver RPC calls, updates `OptimizationJobs` to `RUNNING|SOLVED|FAILED`, and persists `OPTIMIZATION_SOLVED` payloads to Spanner `OutboxEvents` in the same result transaction (no direct Inventory/Manifest mutations).
	- Adapter transport is now fully protobuf-wired in `services/optimizer-core/adapters/go/internal/optimizergrpc/client.go` using generated Go stubs in `services/optimizer-core/adapters/go/internal/optimizerpb/{optimizer_core.pb.go,optimizer_core_grpc.pb.go}` with bidirectional envelope mapping for VRP and CP-SAT requests/responses.
	- Adapter contracts now emit explicit `status` and `matrix_size` in `services/optimizer-core/adapters/go/internal/{model/types.go,adapter/worker.go}`, and `services/optimizer-core/adapters/go/internal/telemetry/metrics.go` plus `cmd/optimizer-worker/main.go` now export `/metrics` and `/healthz` on `OPTIMIZER_WORKER_METRICS_ADDR` (default `:8082`) with `void_optimizer_status_count` and `void_optimizer_latency_matrix_ratio`.
	- Adapter fault-path tests now cover sidecar unavailable, timeout/deadline, and infeasible-result scenarios in `services/optimizer-core/adapters/go/internal/{optimizergrpc/client_test.go,adapter/worker_test.go}`.
	- Phase-7 canary hard gates are now encoded in `infra/k8s/monitoring/prometheusrule.yaml` and `infra/terraform/observability.tf` via `void_optimizer_solver_duration_seconds`, `void_optimizer_error_rate`, and `void_optimizer_circuit_breaker_state`.
	...
	- Local no-cloud simulation is now first-class via Docker: `docker-compose.yml` now bootstraps `pegasus-optimizer-jobs` in `kafka-init`, adds profile `optimizer-sim` services (`optimizer-core-rust`, `optimizer-adapter-worker`), starts Redis as a healthy dependency for worker active-job cleanup parity when `REDIS_ADDRESS=redis:6379` is present, mirrors current K8s resource limits (`optimizer-core-rust` `500m`/`512Mi`, `optimizer-adapter-worker` `1000m`/`1Gi`), and publishes local adapter metrics on host port `8082`; orchestration targets remain `optimizer-sim-up`, `optimizer-sim-down`, and `optimizer-sim-bootstrap` in `Makefile`, which now auto-detects `docker compose` vs legacy `docker-compose` for local bring-up compatibility.
	- Backend-go now provisions and wires the additive async optimizer dispatch path in `apps/backend-go/{schema/spanner.ddl,migrations/migrations.go,cmd/setup/main.go,optimizationjobs/**,cache/{keys.go,optimization_jobs.go},supplier/{dispatcher.go,dispatch_queue.go,dispatch_jobs.go,dispatch_apply.go,manifest.go},kafka/{events.go,notification_dispatcher.go}}` plus `packages/optimizer-contract/jobs.go`: `OptimizationJobs` stores supplier/status/request/idempotency indexed job records, `optimizationjobs` exposes typed queued-job and bounded read/query models with `APPLYING` treated as active, supplier auto-dispatch now snapshots order/driver state into canonical queue envelopes enriched with render-ready depot/node/vehicle metadata, queue-side durable enqueue now performs best-effort `SADD` into Redis set `supplier:{supplier_id}:jobs:active`, supplier read APIs now serve Redis-primary/Spanner-fallback active/projection views backed by `OptimizationJobs.ResultPayload`, terminal `OPTIMIZATION_SOLVED` events now drive supplier-scoped websocket completion frames keyed by `job_id`, backend-go now consumes the same solved event to transition `SOLVED -> APPLYING -> APPLIED` and idempotently materialize draft manifests through deterministic `route_id` / `manifest_id` plus `CreateDraftManifestWithIdentity`, regenerated `contracts/events.schema.json` and `packages/types/ws-events.ts` now expose typed `OptimizationSolvedPayload`, and `apps/admin-portal/app/supplier/dispatch/page.tsx` now hydrates queued results from websocket completion events via `packages/types/fleet.ts` + `apps/admin-portal/lib/api/dispatch-jobs.ts` while holding confirm until apply completes.
	- Optimizer worker runtime now supports optional Redis cleanup for K3 active-job indexing: `services/optimizer-core/adapters/go/internal/{config/config.go,adapter/worker.go}` plus `cmd/optimizer-worker/main.go` honor `REDIS_ADDRESS`, remove supplier job ids from `supplier:{supplier_id}:jobs:active` only after `SOLVED` or `FAILED` Spanner commit succeeds, and degrade cleanly when Redis is unavailable; local `optimizer-sim` compose wiring now passes `REDIS_ADDRESS=redis:6379` to preserve no-cloud parity.
	...
	- Container/runtime scaffolding remains additive through `services/optimizer-core/{Dockerfile.rust,Dockerfile,requirements.txt,scripts/gen_proto.sh}` and Go module bootstrap in `services/optimizer-core/adapters/go/go.mod`; Python sidecar remains available only for reference/parity checks outside the canonical production path.
...
- Phase-3 supplier H3 polygon fill and compacted transport: `pegasus/apps/backend-go/{proximity/{h3.go,zone_preview.go,geo_report.go},supplier/warehouses.go}` now uses native `h3.PolygonToCells` for polygon coverage and exposes additive compacted response fields (`hexes_compacted`, `h3_indexes_compacted`, `h3_resolution`); supplier portal consumers in `pegasus/apps/admin-portal/app/supplier/{_shared/h3-coverage.ts,geo-report/page.tsx,warehouses/{page.tsx,CoverageEditor.tsx}}` now lazily expand compacted arrays via `h3-js uncompactCells`, while Spanner `Warehouses.H3Indexes` and Redis `whcell:*` remain expanded for serving-warehouse, audit, and cache-index stability.

- H3 memory and egress safety hardening: `pegasus/apps/backend-go/proximity/{h3.go,zone_preview.go,geo_report.go}` + `pegasus/apps/backend-go/supplier/warehouses.go`
	- `h3.go` now defines shared coverage policy limits (max polygon points, area ceilings, max generated cells, fallback sampler iteration budget, response-size caps) and typed coverage error contracts.
	- `zone_preview.go` now validates polygon complexity pre-compute, maps guardrail violations to structured `400/422/413` responses, and enforces bounded fallback sampler growth.
	- `supplier/warehouses.go` now reuses the same proximity guardrails for `POST /v1/supplier/warehouses/{id}/coverage` mutations.
	- `geo_report.go` now replaces unbounded overlap-cell aggregation with bounded unique sampling and caps per-warehouse coverage payload arrays through additive `coverage_truncated` metadata.

- H2 observability + H3 pentagon fallback closure: `pegasus/apps/backend-go/proximity/{metrics.go,h3.go,zone_preview.go,geo_report.go,recommendation.go}`
	- `metrics.go` now exports Prometheus `void_spatial_compaction_ratio{zone_type,resolution}` and `void_spatial_egress_overflow_total{zone_type,resolution}`.
	- `zone_preview.go` and `geo_report.go` now observe compacted/raw coverage ratios and enforce compacted egress circuit-breaker truncation at `MaxCompactedEgressCells=5000` with warning logs.
	- `h3.go` now wraps CGO boundary calls (`LatLngToCell`, `GridDisk`, `PolygonToCells`, `GridDistance`) with panic-safe guards, pentagon-neighborhood detection, and Haversine fallback micro-batches.
	- `recommendation.go#H3GridDistance` now routes through the safe grid-distance wrapper to preserve routing/scoring behavior under pentagon boundary failures.
...
- Shared order compatibility route composition: `pegasus/apps/backend-go/orderroutes/routes.go`
	...
	- Card checkout, B2B checkout, and unified checkout now return additive policy metadata (`resolved_gateway`, `policy_source`, `allowed_gateways`, `policy_reason`) while keeping legacy gateway/payment_url fields intact.
	...
	- Payment execution paths enforce those seams end-to-end: `payment/session.go` create/attempt/bind/retry normalize gateway values through `NewProviderClient`, checkout URL generators fail closed for unsupported/unavailable providers, Global Pay webhook settlement validates session gateway via provider resolver, `order/service.go` now routes retailer card checkout plus fulfillment saved-card charge through `ProviderExecutionRouter`, `order/unified_checkout.go#authorizeAtCheckout` now uses it for Global Pay holds, and `payment/refund.go` now resolves provider credentials through `VaultResolver` and refunds against durable session/attempt provider references. Adyen direct execution is now implemented via `payment/adyen.go` + `payment/execution.go` (stored-method charge, manual-capture authorization, capture, void, refund) with bounded retries and idempotency controls; `payment/adyen_webhook.go` now maps CAPTURE/CAPTURE_FAILED/CANCELLATION/REFUND/REFUND_FAILED events into additive `PaymentAttempts` execution metadata transitions.
	- Payment attempts now persist additive execution metadata: `payment/session.go` writes `ExecutionAction` and `ExecutionMode` via `CreateAttempt` and supports post-branch corrections via `UpdateAttemptExecutionMetadata`; `order/service.go` now applies branch-accurate updates in both card-checkout and fulfillment flows by creating `CHECKOUT_INIT`/`AUTO` attempts at initialization and switching to direct (`CHECKOUT_INIT`/`DIRECT_STORED_METHOD`), direct 3DS redirect (`CHECKOUT_INIT`/`DIRECT_3DS_REDIRECT`), or hosted (`HOSTED_CHECKOUT_INIT`/`HOSTED_REDIRECT`) modes before provider execution; schema parity is maintained in `migrations/migrations.go`, `cmd/setup/main.go`, and canonical `schema/spanner.ddl` (`GlobalPayntAttempts`).
	...
	- Deprecated compatibility path `POST /v1/payment/global_pay/initiate` now emits the same structured envelope style plus `migrate_to` guidance (`/v1/order/card-checkout`) while preserving legacy success payload shape.
