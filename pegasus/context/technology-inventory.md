...
- OR-Tools sidecar optimization runtime (Pattern-A): `pegasus/services/optimizer-core/**`
	...
	- Local no-cloud simulation is now first-class via Docker: `docker-compose.yml` now bootstraps `pegasus-optimizer-jobs` in `kafka-init`, adds profile `optimizer-sim` services (`optimizer-core-rust`, `optimizer-adapter-worker`), mirrors current K8s resource limits (`optimizer-core-rust` `500m`/`512Mi`, `optimizer-adapter-worker` `1000m`/`1Gi`), and publishes local adapter metrics on host port `8082`; orchestration targets remain `optimizer-sim-up`, `optimizer-sim-down`, and `optimizer-sim-bootstrap` in `Makefile`.
