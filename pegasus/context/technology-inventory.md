...
- Shared order compatibility route composition: `pegasus/apps/backend-go/orderroutes/routes.go`
	...
	- Payment execution paths now enforce that resolver end-to-end: `payment/session.go` create/attempt/bind/retry normalize gateway values through `NewProviderClient`, checkout URL generators fail closed for unsupported/unavailable providers, Global Pay webhook settlement validates session gateway via provider resolver, and order checkout entry points reject `ADYEN` before mutation side effects while adapter wiring remains unavailable.
	- Webhook route composition now mounts both `POST /v1/webhooks/global-pay` and `POST /v1/webhooks/stripe` through `webhookroutes/routes.go`, preserving signature-first webhook handling behind priority/log middleware wrappers.
