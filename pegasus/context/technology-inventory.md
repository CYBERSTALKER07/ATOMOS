...
- Supplier inventory import staging surface: `pegasus/apps/backend-go/{schema/spanner.ddl,migrations/migrations.go,supplier/imports.go}` + `pegasus/apps/admin-portal/app/supplier/inventory/import/page.tsx`
	...
	- Staged import apply audit writes now use `BULK_IMPORT_STAGED` in `InventoryAuditLog`.
	- Admin portal now exposes `app/supplier/inventory/import/page.tsx` as the bulk-import ingress shell and links it from `app/supplier/inventory/page.tsx`, including drag-drop direct upload, uploaded-signal bridge, and processing scanner/skeleton states.
...
- Shared order compatibility route composition: `pegasus/apps/backend-go/orderroutes/routes.go`
	...
	- Payment execution paths enforce that resolver end-to-end: `payment/session.go` create/attempt/bind/retry normalize gateway values through `NewProviderClient`, checkout URL generators fail closed for unsupported/unavailable providers, Global Pay webhook settlement validates session gateway via provider resolver, and order card checkout now initializes Adyen hosted checkout links through `order/service.go` + `SessionService.BindProviderCheckout`.
