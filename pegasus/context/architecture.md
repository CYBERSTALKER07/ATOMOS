...
- **Realtime and infra split**: `pegasus/apps/backend-go/telemetryroutes/routes.go` owns `/ws/{telemetry,fleet}`, `pegasus/apps/backend-go/warehouseroutes/routes.go` owns `/ws/warehouse`, `pegasus/apps/backend-go/catalogroutes/routes.go` owns legacy `/v1/products`, and `pegasus/apps/backend-go/infraroutes/routes.go` is reduced to infra-only compatibility mounts (`/v1/health` plus development-only `/debug/mint-token`).
