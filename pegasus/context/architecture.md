...
- **Factory role-row parity**: `factoryroutes` owns the expanded factory contract (`/v1/factory/{dashboard,profile,analytics/overview,transfers*,manifests*,fleet*,staff*,dispatch,supply-requests*}`), while factory portal shell now hydrates `/v1/factory/profile`, Android/iOS clients expose additive service endpoints for advanced factory ops routes (`transfers/create`, `manifests/{id}`, `manifests/{id}/{action}`, `fleet/{drivers,vehicles}`, `staff/{id}`), and factory dashboard refresh cadence is aligned to 30s polling across portal, Android, and iOS (no `/v1/ws/factory` channel currently mounted).
