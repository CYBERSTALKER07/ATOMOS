...
- **Structured logging tranche**: runtime logging in `analytics/`, `sync/`, `replenishment/`, `treasury/`, and `vault/` now uses `log/slog` structured events, aligning these high-noise paths with the websocket/kafka structured observability baseline.
