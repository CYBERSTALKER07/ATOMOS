...
class X {
  ...
  #initRenderer() {
    ...
    if (!this.canvas) {
      throw new Error('Ballpit: no canvas element');
    }
    this.canvas.style.display = 'block';
    // Pre-check WebGL availability to avoid cryptic null errors
    const testCtx = this.canvas.getContext('webgl2') || this.canvas.getContext('webgl');
    if (!testCtx) {
      throw new Error('Ballpit: WebGL not available');
    }
  ...
  #onIntersection(entries: IntersectionObserverEntry[]) {
    ...
    if (this.#isAnimating) { this.#startAnimation(); } else { this.#stopAnimation(); }
  }

  #onVisibilityChange() {
    if (this.#isAnimating) {
      if (document.hidden) { this.#stopAnimation(); } else { this.#startAnimation(); }
    }
  }
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      try {
        ...
        console.warn('Ballpit: WebGL init failed', e);
      }
    }
    ...
    requestAnimationFrame(() => {
      init();
      if (!spheresInstanceRef.current && !disposed) {
        // Parent may not be sized yet — watch for resize
        const parent = canvas.parentElement;
        if (parent) {
          const ro = new ResizeObserver(() => {
            init();
            if (spheresInstanceRef.current) ro.disconnect();
          });
          ro.observe(parent);
          // Store for cleanup
          (canvas as any).__ballpitRO = ro;
        }
      }
    });
    ...
    return () => {
      disposed = true;
      const ro = (canvas as any).__ballpitRO as ResizeObserver | undefined;
      if (ro) { ro.disconnect(); delete (canvas as any).__ballpitRO; }
