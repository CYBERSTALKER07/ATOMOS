...
func (cs *ChargebackService) HandleReversal(ctx context.Context, sessionID string) error {
	// Read the session to get order and amount details
	row, err := cs.spanner.Single().ReadRow(ctx, "PaymentSessions",
	...
	var sessID, orderID, retailerID, status, gateway string
	...
	_, txErr := cs.spanner.ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
		// Mark session as reversed
		m1 := spanner.Update("PaymentSessions",
			[]string{"SessionId", "Status", "UpdatedAt"},
			[]interface{}{sessionID, "REVERSED", spanner.CommitTimestamp},
		)
