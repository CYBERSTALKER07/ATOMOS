...
var providerRegistry = map[string]ProviderCapability{
        ...
        "STRIPE": {
                Gateway:        "STRIPE",
                DisplayName:    "Stripe",
                OnboardingMode: RedirectPlusManual,
                RequiredFields: []string{"stripe_account_id"},
                ConnectLabel:   "Connect with Stripe",
                ConnectHint:    "Click to set up or link your Stripe account.",
                ManualHint:     "Stripe requires the OAuth merchant-connect flow. Contact support if you need manual entry.",
        },
        "ADYEN": {
                Gateway:        "ADYEN",
                DisplayName:    "Adyen",
                OnboardingMode: ManualOnly,
                RequiredFields: []string{"merchant_account", "api_key", "client_key"},
                ManualFields: []ManualField{
                        {Name: "merchant_account", Label: "Merchant Account", Placeholder: "Your Adyen Merchant Account Name"},
                        {Name: "api_key", Label: "API Key", Placeholder: "Enter Web Service User API Key", InputType: "password"},
                        {Name: "client_key", Label: "Client Key", Placeholder: "Enter Client Key for Drop-in components"},
                },
                ManualHint: "Enter your Adyen integration credentials from your Customer Area.",
        },
}

// providerOrder enforces display order in the UI.
var providerOrder = []string{"CASH", "GLOBAL_PAY", "STRIPE", "ADYEN"}
