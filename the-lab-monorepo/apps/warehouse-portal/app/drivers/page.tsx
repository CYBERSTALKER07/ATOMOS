      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {drivers.map(d => (
                <tr key={d.driver_id} className="border-b border-[var(--border)] hover:bg-[var(--surface)] transition-colors">
                  <td className="py-2.5 px-3 font-medium">{d.name}</td>
                  <td className="py-2.5 px-3 text-[var(--muted)]">{d.phone}</td>
                  <td className="py-2.5 px-3">
                    <span className={`status-chip ${d.truck_status === 'ON_ROUTE' ? 'status-chip--active' : 'status-chip--stable'}`}>
                      {d.truck_status || 'IDLE'}
                    </span>
                  </td>
                  <td className="py-2.5 px-3">
