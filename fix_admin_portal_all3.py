...
def fix_all():
    ...
    for root, _, files in os.walk("the-lab-monorepo/apps/admin-portal/components"):
        for file in files:
            if file.endswith(".tsx"):
                rel_path = os.path.relpath(os.path.join(root, file), "the-lab-monorepo/apps/admin-portal/")
                regex_replace_in_file(rel_path, r'\bisLoading=\{', 'isPending={')
    ...
    # 2. Duplicate keys in billing/page.tsx
    ...
    regex_replace_in_file("app/supplier/supply-lanes/page.tsx", r"style=\{\{ background: c\.bg, color: c\.text, '--ring-color': c\.text \}\}", "style={{ background: c.bg, color: c.text, '--ringColor': c.text } as any}")
    regex_replace_in_file("app/supplier/supply-lanes/page.tsx", r"'--ring-color': c\.text", "'--ringColor': c.text")
