...
Runtime additive note (2026-05-20): Pattern-A OR-Tools scaffold now lives under `pegasus/services/optimizer-core`: Python gRPC sidecar (`server/{vrp_adapter.py,cpsat_adapter.py,service.py,main.py}`) provides VRP + CP-SAT solvers with deterministic UUID-index mapping and `SCALE_FACTOR=10000` integer adapters, while Go worker scaffold (`adapters/go/**`) consumes Kafka optimization jobs, applies bounded retry/backoff+jitter around solver calls, and writes `OPTIMIZATION_SOLVED` payloads to Spanner `OutboxEvents` only (no direct Inventory/Manifest mutations).
