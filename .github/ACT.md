...
16. Supplier planning gate: treat `/v1/supplier/delivery-zones*`, `/v1/supplier/factories*`, `/v1/supplier/geocode/reverse`, `/v1/supplier/retailers/locations`, `/v1/supplier/supply-lanes*`, `/v1/supplier/network-{mode,analytics}`, `/v1/supplier/replenishment/{kill-switch,audit,pull-matrix,predictive-push}`, and `/v1/supplier/warehouses/{territory-preview,apply-territory}` as one contract owned by `pegasus/apps/backend-go/supplierplanningroutes/routes.go`; supplier factory create/list/detail/profile payloads are expected to carry additive `h3_index` and `product_types` metadata for the admin-portal planning consumers, and supply-lane mutate actions must resolve the organisation-level supplier scope via `claims.ResolveSupplierID()` while honoring the published `PATCH /v1/supplier/supply-lanes/{id}` contract, so check the supplier map, factory-planning, territory, supply-lane, and replenishment-control consumers before changing one path.
17. Supplier core gate: treat `/v1/supplier/dashboard`, `/v1/supplier/earnings`, `/v1/supplier/inventory*`, and `/v1/supplier/orders*` as one contract owned by `pegasus/apps/backend-go/suppliercoreroutes/routes.go`; supplier inventory is expected to honor the mounted root `PATCH /v1/supplier/inventory` contract and publish additive `sku_id`/`product_name` aliases for portal consumers, so check the supplier dashboard, earnings, inventory, and order-vetting consumers before changing one path.
18. Retailer role-row gate: treat `/v1/retailer/analytics/*`, `/v1/orders/request-cancel`, `/v1/order/{create,cancel,cash-checkout,card-checkout}`, `/v1/retailer/shop-closed-response`, `/v1/retailer/family-members*`, `/v1/retailer/orders/{confirm-ai,reject-ai}`, `/v1/orders/{edit-preorder,confirm-preorder}`, `/v1/retailer/cart/sync`, `/v1/retailer/suppliers*`, `/v1/retailer/profile`, `/v1/retailers/{retailerID}/orders`, `/v1/retailer/{tracking,cards,pending-payments,active-fulfillment}`, `/v1/retailer/card/{initiate,confirm,deactivate,default}`, `/v1/retailer/settings/auto-order*`, and `/v1/ws/retailer` as one contract owned by `pegasus/apps/backend-go/retailerroutes/routes.go`; treat shared `/v1/orders*` and `/v1/order/refunds` as the adjacent compatibility contract owned by `pegasus/apps/backend-go/orderroutes/routes.go`; check retailer desktop, retailer iOS, and retailer Android consumers together before changing one path because those clients already depend on overlapping but not identical slices of this route family.
19. Simulation harness gate: treat `/v1/internal/sim/{start,stop,status}` as one contract owned by `pegasus/apps/backend-go/simroutes/routes.go`; keep role guard ADMIN-only, preserve boot-time `SIMULATION_ENABLED` gating through `app.Simulation`, and keep all handler logic in `pegasus/apps/backend-go/simulation/handler.go`.
20. Driver role-row gate: treat `/v1/driver/{earnings,history,availability,pending-collections,profile,manifest-gate,manifest}`, `/v1/fleet/manifest` (legacy alias), and `/v1/ws/driver` as one contract owned by `pegasus/apps/backend-go/driverroutes/routes.go`; treat adjacent execution routes in `fleetroutes` and shared order detail/state reads in `orderroutes` as part of the same mobile role-row contract, check both `driver-app-android` and `driverappios` consumers before changing one path, keep high-consequence driver mutations (`/v1/order/deliver`, `/v1/order/confirm-offload`, `/v1/order/complete`, `/v1/order/collect-cash`, `/v1/delivery/arrive`) on deterministic `Idempotency-Key` headers across both clients, and keep `/v1/driver/profile` / driver-login payloads additive for dual-node operations (`home_node_type`, `home_node_id`, `driver_mode`, and factory-node metadata) so one app pair can cover factory-transfer and warehouse-delivery drivers without app forks.
21. Payload role-row gate: treat `/v1/payloader/{trucks,orders,recommend-reassign}`, `/v1/payload/seal`, and `/v1/ws/payloader` as one contract owned by `pegasus/apps/backend-go/payloaderroutes/routes.go`; treat shared manifest lifecycle paths in `supplierlogisticsroutes`, `POST /v1/delivery/missing-items` in `deliveryroutes` (driver+payloader; additive payload compatibility for `items` alias and `source=PAYLOAD_TERMINAL` empty-item flags), reassign in `fleetroutes`, and inbox/token paths in `userroutes` as adjacent payload contracts, and check payload-terminal, payload-app-ios, and payload-app-android consumers together before changing one path.
