...
17. Supplier core gate: treat `/v1/supplier/dashboard`, `/v1/supplier/earnings`, `/v1/supplier/inventory*`, and `/v1/supplier/orders*` as one contract owned by `pegasus/apps/backend-go/suppliercoreroutes/routes.go`; check the supplier dashboard, earnings, inventory, and order-vetting consumers before changing one path.
