...
16. Supplier planning gate: treat `/v1/supplier/delivery-zones*`, `/v1/supplier/factories*`, `/v1/supplier/geocode/reverse`, `/v1/supplier/retailers/locations`, `/v1/supplier/supply-lanes*`, `/v1/supplier/network-{mode,analytics}`, `/v1/supplier/replenishment/{kill-switch,audit,pull-matrix,predictive-push}`, and `/v1/supplier/warehouses/{territory-preview,apply-territory}` as one contract owned by `pegasus/apps/backend-go/supplierplanningroutes/routes.go`; check the supplier map, factory-planning, territory, supply-lane, and replenishment-control consumers before changing one path.
