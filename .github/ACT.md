...
20. Driver role-row gate: treat `/v1/driver/{earnings,history,availability,pending-collections,profile,manifest-gate,manifest}`, `/v1/fleet/manifest` (legacy alias), and `/v1/ws/driver` as one contract owned by `pegasus/apps/backend-go/driverroutes/routes.go`; treat adjacent execution routes in `fleetroutes` and shared order detail/state reads in `orderroutes` as part of the same mobile role-row contract, check both `driver-app-android` and `driverappios` consumers before changing one path, and keep high-consequence driver mutations (`/v1/order/deliver`, `/v1/order/confirm-offload`, `/v1/order/complete`, `/v1/order/collect-cash`, `/v1/delivery/arrive`) on deterministic `Idempotency-Key` headers across both clients.
21. Payload role-row gate: treat `/v1/payloader/{trucks,orders,recommend-reassign}`, `/v1/payload/seal`, and `/v1/ws/payloader` as one contract owned by `pegasus/apps/backend-go/payloaderroutes/routes.go`; treat shared manifest lifecycle paths in `supplierlogisticsroutes`, `POST /v1/delivery/missing-items` in `deliveryroutes` (driver+payloader), reassign in `fleetroutes`, and inbox/token paths in `userroutes` as adjacent payload contracts, and check payload-terminal, payload-app-ios, and payload-app-android consumers together before changing one path.
