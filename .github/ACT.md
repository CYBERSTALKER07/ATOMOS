...
16. Supplier planning gate: treat `/v1/supplier/delivery-zones*`, `/v1/supplier/factories*`, `/v1/supplier/geocode/reverse`, `/v1/supplier/retailers/locations`, `/v1/supplier/supply-lanes*`, `/v1/supplier/network-{mode,analytics}`, `/v1/supplier/replenishment/{kill-switch,audit,pull-matrix,predictive-push}`, and `/v1/supplier/warehouses/{territory-preview,apply-territory}` as one contract owned by `pegasus/apps/backend-go/supplierplanningroutes/routes.go`; supplier factory create/list/detail/profile payloads are expected to carry additive `h3_index` and `product_types` metadata for the admin-portal planning consumers, so check the supplier map, factory-planning, territory, supply-lane, and replenishment-control consumers before changing one path.
...
18. Retailer core gate: treat `/v1/retailer/analytics/*`, `/v1/orders/request-cancel`, `/v1/retailer/shop-closed-response`, `/v1/retailer/family-members*`, `/v1/retailer/orders/{confirm-ai,reject-ai}`, `/v1/orders/{edit-preorder,confirm-preorder}`, `/v1/retailer/cart/sync`, `/v1/retailer/suppliers*`, `/v1/retailer/profile`, `/v1/retailers/{retailerID}/orders`, and `/v1/retailer/tracking` as one contract owned by `pegasus/apps/backend-go/retailerroutes/routes.go`; check retailer desktop, retailer iOS, and retailer Android consumers together before changing one path because those clients already depend on overlapping but not identical slices of this route family.
