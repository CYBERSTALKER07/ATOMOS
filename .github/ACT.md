...
21. Driver role-row gate: treat `/v1/driver/{earnings,history,availability,pending-collections,profile,manifest-gate,manifest}`, `/v1/fleet/manifest` (legacy alias), and `/v1/ws/driver` as one contract owned by `pegasus/apps/backend-go/driverroutes/routes.go`; treat adjacent execution routes in `fleetroutes` and shared order detail/state reads in `orderroutes` as part of the same mobile role-row contract, check both `driver-app-android` and `driverappios` consumers before changing one path, keep high-consequence driver mutations (`/v1/order/deliver`, `/v1/order/confirm-offload`, `/v1/order/complete`, `/v1/order/collect-cash`, `/v1/delivery/arrive`) on deterministic `Idempotency-Key` headers across both clients, and keep `/v1/driver/profile` / driver-login payloads additive for dual-node operations (`home_node_type`, `home_node_id`, `driver_mode`, and factory-node metadata) so one app pair can cover factory-transfer and warehouse-delivery drivers without app forks.
