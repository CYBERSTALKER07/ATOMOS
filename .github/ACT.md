...
18. Retailer role-row gate: treat `/v1/retailer/analytics/*`, `/v1/orders/request-cancel`, `/v1/order/{cash-checkout,card-checkout}`, `/v1/retailer/shop-closed-response`, `/v1/retailer/family-members*`, `/v1/retailer/orders/{confirm-ai,reject-ai}`, `/v1/orders/{edit-preorder,confirm-preorder}`, `/v1/retailer/cart/sync`, `/v1/retailer/suppliers*`, `/v1/retailer/profile`, `/v1/retailers/{retailerID}/orders`, `/v1/retailer/{tracking,cards,pending-payments,active-fulfillment}`, `/v1/retailer/card/{initiate,confirm,deactivate,default}`, `/v1/retailer/settings/auto-order*`, and `/v1/ws/retailer` as one contract owned by `pegasus/apps/backend-go/retailerroutes/routes.go`; check retailer desktop, retailer iOS, and retailer Android consumers together before changing one path because those clients already depend on overlapping but not identical slices of this route family.
