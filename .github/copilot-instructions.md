...
12. **Phase VIII Replenishment scaffolding** — PARTIALLY CLOSED: `EventReplenishmentLockAcquired` / `EventReplenishmentLockReleased` are now produced by `factory/replenishment_lock.go` and consumed in `kafka/notification_dispatcher.go`. `EventStockThresholdBreach` and `EventLookAheadCompleted` now have notification consumers wired, but producers are still pending.
