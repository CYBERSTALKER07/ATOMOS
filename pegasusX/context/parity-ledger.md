...
- 2026-06-29: Payload ecosystem gap closure — `POST /v1/payload/manifest-exception` now uses Redis idempotency replay (terminal/Android/iOS already sent keys); shared `returns` inbound scan/confirm handlers wired with the same Redis store (warehouse + payload clients already sent `warehouseInboundScanKey` / `payloadInboundScanKey`); `payloadManifestExceptionKey` exported from `@pegasusx/api-client`; terminal re-dispatch apply confirmed on `POST /v1/payloader/reassign-order` (batch `fleet/reassign` retained for SSMR only).
...
- 2026-06-29: Driver ecosystem gap closure — availability mutations (`POST/PATCH /v1/driver/availability`) and order state PATCH (`PATCH /v1/orders/{id}/state`) now use Redis idempotency replay on the driver service; `driverAvailabilityKey` aligned on Android `DriverIdempotencyKeys` + iOS `DriverIdempotency` for session-end / availability calls (delivery edges were already keyed).
