...
- **Status:** `implemented` (PX8-A3, PX11-B2, PX12-H) per ledgers; thinner than portal by design.
...
- **Status:** Shells + basic wiring `implemented`; full feature parity with portal for ops surfaces advancing with current dispatch work.
...
- **Status:** Core + edges + telemetry `E2E_SSMR_GREEN` / `implemented` (PX5, PX12-B/F). Design parity closed with pegasusX ahead on connection UX.
