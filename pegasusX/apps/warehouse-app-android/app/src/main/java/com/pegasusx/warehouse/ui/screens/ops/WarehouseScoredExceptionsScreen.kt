...
import com.pegasusx.warehouse.data.model.ScoredException
import com.pegasusx.warehouse.data.model.ScoredExceptionsResponse
...
@Composable
fun WarehouseScoredExceptionsScreen(
    api: WarehouseApi,
    onBack: (() -> Unit)?,
) {
    ...
    fun load() {
        ...
        scope.launch {
            try {
                val resp = api.scoredControlTower()
                    ...
                    ElevatedCard(Modifier.fillMaxWidth()) {
                        Column(
                            ...
                            if (row.topPlaybookName.isNotBlank()) {
                                Text(row.topPlaybookName, style = MaterialTheme.typography.bodySmall)
                            }
                        }
                    }
                }
