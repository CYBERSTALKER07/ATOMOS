...
import com.pegasusx.warehouse.data.model.ClientPolicyResponse
...
/**
 * Website-only enterprise OTA for warehouse Android.
 */
class AutoUpdater(private val context: Context) {
    private val downloadManager =
        context.getSystemService(Context.DOWNLOAD_SERVICE) as DownloadManager
    private val tag = "WarehouseAutoUpdater"
    private val prefs = context.getSharedPreferences("warehouse_enterprise_updater", Context.MODE_PRIVATE)
    ...
    private fun startDownload(manifest: Manifest) {
        Log.i(tag, "Downloading warehouse enterprise update ${manifest.versionCode} ${manifest.apkUrl}")
        val request = DownloadManager.Request(Uri.parse(manifest.apkUrl))
            .setTitle("Warehouse Update ${manifest.versionName.ifBlank { manifest.versionCode.toString() }}")
            .setDescription(manifest.notes.ifBlank { "Enterprise website update" })
            .setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
            .setDestinationInExternalFilesDir(context, Environment.DIRECTORY_DOWNLOADS, "warehouse-update.apk")
    ...
    private fun verifyAndInstall(downloadId: Long, expectedHash: String) {
        ...
        try {
            ...
            val localUri = cursor.getString(cursor.getColumnIndexOrThrow(DownloadManager.COLUMN_LOCAL_URI))
                ?: return
            val dest = File(context.cacheDir, "warehouse-update-install.apk")
