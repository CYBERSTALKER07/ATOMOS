...
export default function ReplenishmentPoliciesPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {
        if (!cancelled) setError(t("supplier_portal.residual.text.load_replenishment_policies_failed"));
      })
      .finally(() => {
  ...
  async function saveSafetyStock() {
    ...
    try {
      ...
      setError(t("supplier_portal.residual.text.save_replenishment_policies_failed"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <PageChrome
      icon="inventory"
      title={t("supplier_portal.operations.replenishment_policies.text.replenishment_policies")}
      description={t("supplier_portal.residual.text.auto_approval_thresholds_transfer_limits_and_service_level_safet")}
      loading={loading}
      ...
      {policy ? (
        <div className="flex flex-col gap-6">
          <KpiStatGrid columns={2}>
            <KpiStatCard
              label={t("supplier_portal.residual.text.auto_approve_stable")}
              value={policy.auto_approve_stable ? "Enabled" : "Disabled"}
            />
            <KpiStatCard
              label={t("supplier_portal.residual.text.auto_approve_predictive_push")}
              value={policy.auto_approve_predictive_push ? "Enabled" : "Disabled"}
            />
            <KpiStatCard label={t("supplier_portal.residual.text.max_daily_transfer_units")} value={policy.max_daily_transfer_units} />
            <KpiStatCard label={t("supplier_portal.residual.text.min_confidence_score")} value={policy.min_confidence_score} />
          </KpiStatGrid>

          <section className="desk-card p-6 flex flex-col gap-4">
            <div>
              <h2 className="md-typescale-title-medium">{t("supplier_portal.operations.replenishment_policies.text.safety_stock_8_2")}</h2>
