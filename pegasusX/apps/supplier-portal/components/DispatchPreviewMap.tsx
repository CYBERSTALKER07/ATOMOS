...
type DispatchPreviewMapProps = {
  routes: Array<DispatchProposedRoute | { route_geometry?: RouteGeometryWire; driver_name?: string; driver_id?: string }>;
  className?: string;
};
...
function toLineFeature(
  route: DispatchPreviewMapProps['routes'][number],
  index: number,
...
export default function DispatchPreviewMap({ routes, className }: DispatchPreviewMapProps) {
  ...
  const featureCollection = useMemo<RouteFeatureCollection>(() => {
    const features = routes
      .map((route, index) => toLineFeature(route, index))
