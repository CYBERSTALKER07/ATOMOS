...
final class FleetDriverMarkerAnimator {
    ...
    func updateTargets(_ routes: [SupplierFleetLiveRoute]) {
        var active = Set<String>()
        for route in routes {
            guard route.liveLocationAvailable, let location = route.driverLocation else { continue }
            let lat = location.latitude != 0 ? location.latitude : location.lat
            let lng = location.longitude != 0 ? location.longitude : location.lng
            ...
            if var existing = states[route.driverId] {
                if existing.toLat != lat || existing.toLng != lng {
                    ...
                    existing.fromLat = lerp(existing.fromLat, existing.toLat, progress)
                    existing.fromLng = lerp(existing.fromLng, existing.toLng, progress)
                    existing.toLat = lat
                    existing.toLng = lng
