...
// 3-step Factory onboarding wizard.
// HARD PRODUCT INVARIANT: never move business/location/payment setup back into this form. 
// That complex setup lives at /setup/factory post-registration.
  ...
  return (
    <div className="auth-card">
      ...
      <footer className="mt-6 flex items-center justify-between gap-4">
        <button type="button" className="portal-btn portal-btn--ghost" onClick={back} disabled={stepIndex === 0 || submitting || stepBusy}>
          Back
        </button>
        {state.step !== "profile" ? (
          <button type="button" className="portal-btn portal-btn--primary" onClick={next} disabled={submitting || stepBusy}>
      ...
      <div className="grid grid-cols-[160px,1fr] gap-3">
        <PortalField id="countryCode" label="Country" error={errors.countryCode}>
          <PortalSelect
            ...
            onChange={(e: ChangeEvent<HTMLSelectElement>) => setState((s) => ({ ...s, identity: { ...s.identity, countryCode: e.target.value } }))}
        ...
        <PortalField id="phoneLocal" label="Phone" error={errors.phoneLocal} hint={`Will be sent as ${dialCode}${state.identity.phoneLocal || "…"}`}>
          <div className="flex">
            ...
            <PortalInput
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => setState((s) => ({ ...s, identity: { ...s.identity, phoneLocal: e.target.value.replace(/\D/g, "") } }))}
  ...
  return (
    <PortalField id="otpCode" label="Verification code" error={errors.otpCode} hint="Enter the 6-digit code sent via SMS.">
      <PortalInput
        ...
        maxLength={6}
        onChange={(e: ChangeEvent<HTMLInputElement>) => setState((s) => ({ ...s, verification: { ...s.verification, otpCode: e.target.value.replace(/\D/g, "") } }))}
  ...
  return (
    <div className="grid gap-4">
      <PortalField id="legalName" label="Legal company name" error={errors.legalName}>
        <PortalInput id="legalName" value={state.profile.legalName} onChange={(e: ChangeEvent<HTMLInputElement>) => setState((s) => ({ ...s, profile: { ...s.profile, legalName: e.target.value } }))} error={errors.legalName} />
      </PortalField>
      <PortalField id="contactName" label="Primary contact name" error={errors.contactName}>
        <PortalInput id="contactName" value={state.profile.contactName} onChange={(e: ChangeEvent<HTMLInputElement>) => setState((s) => ({ ...s, profile: { ...s.profile, contactName: e.target.value } }))} error={errors.contactName} />
      </PortalField>
      <PortalField id="email" label="Work email" error={errors.email}>
        <PortalInput id="email" type="email" autoComplete="email" value={state.profile.email} onChange={(e: ChangeEvent<HTMLInputElement>) => setState((s) => ({ ...s, profile: { ...s.profile, email: e.target.value } }))} error={errors.email} />
      </PortalField>
    </div>
  );
}
