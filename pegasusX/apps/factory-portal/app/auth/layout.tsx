...
export default function AuthLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`auth-shell px-auth-shell ${isDark ? 'auth-dark' : 'auth-light'}`}>
      ...
      <aside className="auth-brand-panel px-auth-brand" aria-label={t("factory_portal.auth.text.pegasusx_factory")}>
        <div className={`auth-brand-content px-auth-brand-content ${mounted ? 'auth-brand-enter' : ''}`}>
          <BrandMark size={88} />
          <div className="px-auth-brand-copy">
            <p className="px-auth-brand-eyebrow">{t("factory_portal.residual.text.pegasusx")}</p>
            <h1 className="px-auth-brand-title">{t("factory_portal.auth.text.factory_node")}</h1>
            <p className="px-auth-brand-sub">
              Loading bay, transfers, and dispatch for your factory floor — one account, one node.
            </p>
          </div>
        </div>
        <p className="auth-brand-footer px-auth-brand-footer">{t("factory_portal.auth.text.pegasusx_and_copy_2026")}</p>
      </aside>

      <div className="auth-form-panel px-auth-form">
        <div className="px-auth-form-top">
          <p className="px-auth-form-kicker lg:hidden">{t("factory_portal.auth.text.pegasusx_factory")}</p>
          <ThemeToggle
            isDark={isDark}
