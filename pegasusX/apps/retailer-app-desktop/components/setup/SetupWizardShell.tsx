...
function SetupStepList({
  ...
  return (
    <ol className="setup-step-list" aria-label={t("retailer_desktop.setup.setup_wizard_shell.text.onboarding_progress")}>
...
export default function SetupWizardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="setup-shell">
      <aside className="setup-rail" aria-label={t("retailer_desktop.setup.setup_wizard_shell.text.setup_progress")}>
        <div>
          <div className="setup-rail-brand">
            ...
            <div>
              <h2 className="setup-rail-title">{t("retailer_desktop.setup.setup_wizard_shell.text.retailer_setup")}</h2>
              <p className="setup-rail-sub">
                Complete your company profile so catalog, checkout, and delivery routing can start.
              </p>
            </div>
          </div>
          <SetupStepList currentIndex={currentIndex} />
        </div>
        <p className="setup-rail-footer">{t("retailer_desktop.auth.text.pegasusx_and_copy_2026")}</p>
      </aside>
