...
export function LocationPicker({ value, onChange, label }: LocationPickerProps) {
  const t = usePortalT();
  const resolvedLabel = label ?? t("retailer_desktop.residual.text.address");
  ...
  const useMyLocation = () => {
    if (!navigator.geolocation) {
      setError(t("retailer_desktop.residual.text.geolocation_is_not_supported_in_this_browser"));
      return;
    }
    setLocating(true);
    setError(null);
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        else setError(t("retailer_desktop.residual.text.could_not_resolve_your_location_to_an_address"));
      },
      () => {
        setLocating(false);
        setError(t("retailer_desktop.residual.text.location_permission_denied"));
      },
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {value.address ? (
          <span className="text-xs text-[var(--desk-text-tertiary)]">{t("retailer_desktop.location_picker.text.saved_for_delivery_routing")}</span>
        ) : null}
      </div>
