...
/// Website-only enterprise OTA for factory iOS (enterprise / ad-hoc distribution).
final class AutoUpdater {
    ...
    func promptInstall(manifest: Manifest? = nil, force: Bool = false) {
        ...
        let notes = target.notes.isEmpty
            ? "A new enterprise factory build (\(target.versionName.isEmpty ? "\(target.versionCode)" : target.versionName)) is available."
            : target.notes
