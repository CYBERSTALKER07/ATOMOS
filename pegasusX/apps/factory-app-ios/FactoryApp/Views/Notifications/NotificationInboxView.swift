...
struct NotificationInboxView: View {
    ...
    @State private var items: [FactoryNotificationItem] = []
        ...
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItemGroup(placement: .topBarTrailing) {
                    if unreadCount > 0 {
                        Button("mobile_factory.ui.mark_all_read") {
    ...
    private func load() {
        ...
        Task {
            defer { loading = false }
            do {
                let page: FactoryNotificationsPage = try await APIClient.shared.get(
...
private struct FactoryNotificationsPage: Decodable {
    let notifications: [FactoryNotificationItem]
    let unreadCount: Int
    let hasMore: Bool
...
struct FactoryNotificationItem: Decodable, Identifiable {
    let id: String
    let type: String
    let title: String
    let body: String
