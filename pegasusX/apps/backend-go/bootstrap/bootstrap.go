...
func NewApp(ctx context.Context, cfg *Config) (*App, error) {
	...
	if redisEnabled {
		if rs, err := idempotency.NewRedisStoreFromAddr(cfg.RedisAddr); err != nil {
			log.Warn("idempotency redis store init failed; using in-memory", "err", err)
		} else {
			idemStore = rs
	...
	if cfg.RequireInfraAdapters && spannerClient == nil {
