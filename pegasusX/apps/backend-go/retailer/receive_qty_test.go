...
func TestReceivableQty_ExcludesRemaining(t *testing.T) {
	// order 10, delivered 7, remaining 3 → receive max 7
	got := ReceivableQty(10, 7, 3, 0, 0)
	if got != 7 {
		t.Fatalf("got %d want 7 (delivered, remaining already excluded from delivered)", got)
	}
