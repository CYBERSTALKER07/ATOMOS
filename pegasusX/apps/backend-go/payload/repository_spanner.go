...
// SpannerRepository provides a Spanner transaction seam for payload outbox events.
// Note: Payload domain entities remain in-memory in pegasusX scaffold for now,
...
func (r *SpannerRepository) Apply(ctx context.Context, mutate func() error, emit func(outbox.TxnBuffer) error) error {
	...
	// Scaffold note: payload state is in-memory and not retry-safe.
	...
	if err != nil {
		return fmt.Errorf("payload outbox transaction: %w", err)
	}
