...
import { autocompleteAddress, resolvePlace, reverseGeocode, type ResolvedLocation } from "@/lib/geocode";
  ...
  const useMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        setLocating(false);
        if (loc) applyResolved(loc);
        else setError("Could not resolve address.");
      },
      () => {
        setLocating(false);
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        {open && suggestions.length > 0 ? (
          <ul className="absolute z-20 mt-1 max-h-48 w-full overflow-auto rounded-lg border border-[var(--border)] bg-[var(--surface)] shadow-lg">
            {suggestions.map((s) => (
      ...
      <button type="button" className="rounded-md border border-[var(--border)] px-3 py-1.5 text-xs" onClick={useMyLocation} disabled={locating}>
