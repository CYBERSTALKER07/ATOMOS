package com.pegasusx.factory.service
...
import com.pegasusx.factory.data.model.ClientPolicyResponse
...
/**
 * Website-only enterprise OTA for factory Android.
 */
class AutoUpdater(private val context: Context) {
    private val downloadManager =
        context.getSystemService(Context.DOWNLOAD_SERVICE) as DownloadManager
    private val tag = "FactoryAutoUpdater"
    private val prefs = context.getSharedPreferences("factory_enterprise_updater", Context.MODE_PRIVATE)
    ...
    private fun startDownload(manifest: Manifest) {
        Log.i(tag, "Downloading factory enterprise update ${manifest.versionCode} ${manifest.apkUrl}")
        val request = DownloadManager.Request(Uri.parse(manifest.apkUrl))
            .setTitle("Factory Update ${manifest.versionName.ifBlank { manifest.versionCode.toString() }}")
            .setDescription(manifest.notes.ifBlank { "Enterprise website update" })
            .setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
            .setDestinationInExternalFilesDir(context, Environment.DIRECTORY_DOWNLOADS, "factory-update.apk")
    ...
    private fun verifyAndInstall(downloadId: Long, expectedHash: String) {
        ...
        try {
            ...
            val localUri = cursor.getString(cursor.getColumnIndexOrThrow(DownloadManager.COLUMN_LOCAL_URI))
                ?: return
            val dest = File(context.cacheDir, "factory-update-install.apk")
