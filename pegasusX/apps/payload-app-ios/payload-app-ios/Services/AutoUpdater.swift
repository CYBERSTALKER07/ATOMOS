...
/// Website-only enterprise OTA for payload iOS (enterprise / ad-hoc distribution).
enum AutoUpdater {
    ...
    private static var lastManifest: Manifest?
    ...
    @MainActor
    static func promptInstall(manifest: Manifest? = nil, force: Bool = false) {
        if !EnterpriseUpdateConfig.enableCdnOta {
        ...
        let notes = target.notes.isEmpty
            ? "A new enterprise payload build (\(target.versionName.isEmpty ? "\(target.versionCode)" : target.versionName)) is available."
            : target.notes
    ...
    static func triggerOTAInstall(manifestURL: URL) {
        if !EnterpriseUpdateConfig.enableCdnOta {
    ...
    @MainActor
    static func openStoreListing(force: Bool = false) {
    ...
    private static func fetchManifest(from urlString: String) async -> Manifest? {
