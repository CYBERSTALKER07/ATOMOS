...
fun HandoffInboxCard(
    ...
    Surface(
        ...
        Column(
            ...
            Text(metadata.title, style = MaterialTheme.typography.labelLarge, fontWeight = FontWeight.SemiBold)
            metadata.subtitle?.takeIf { it.isNotBlank() }?.let {
                Text(it, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
            metadata.fields?.forEach { (key, value) ->
                Text(stringResource(R.string.mobile_driver_ui_replace_value, key.replace('_', ' '), value),
