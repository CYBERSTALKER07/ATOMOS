...
/**
 * Website-only enterprise OTA for driver Android.
 *
...
class AutoUpdater(private val context: Context) {
    ...
    private val tag = "DriverAutoUpdater"
    private val prefs = context.getSharedPreferences("driver_enterprise_updater", Context.MODE_PRIVATE)
    ...
    private fun startDownload(manifest: Manifest) {
        Log.i(tag, "Downloading driver enterprise update ${manifest.versionCode} ${manifest.apkUrl}")
        val request = DownloadManager.Request(Uri.parse(manifest.apkUrl))
            .setTitle("Driver Update ${manifest.versionName.ifBlank { manifest.versionCode.toString() }}")
            .setDescription(manifest.notes.ifBlank { "Enterprise website update" })
            .setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
            .setDestinationInExternalFilesDir(context, Environment.DIRECTORY_DOWNLOADS, "driver-update.apk")
    ...
    private fun verifyAndInstall(downloadId: Long, expectedHash: String) {
        ...
        try {
            ...
            val localUri = cursor.getString(cursor.getColumnIndexOrThrow(DownloadManager.COLUMN_LOCAL_URI))
                ?: return
            val dest = File(context.cacheDir, "driver-update-install.apk")
