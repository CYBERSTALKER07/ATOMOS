...
/// Website-only enterprise OTA for driver iOS (enterprise / ad-hoc distribution).
enum AutoUpdater {
    ...
    static func promptInstall(manifest: Manifest? = nil, force: Bool = false) {
        ...
        let notes = target.notes.isEmpty
            ? "A new enterprise driver build (\(target.versionName.isEmpty ? "\(target.versionCode)" : target.versionName)) is available."
            : target.notes
