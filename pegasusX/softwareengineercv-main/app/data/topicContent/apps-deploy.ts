...
export const appsDeployTopics = {
  'mobile-apps': seedContent({
    ...
    outcomes: ['Kotlin and Swift apps per role', 'Phone SMS login for field roles', 'automatic screen refresh'],
  ...
  'desktop-apps': seedContent({
    title: 'Desktop Apps',
    summary: 'Retailer desktop app for store counters — catalog, checkout, and tracking.',
    problem: 'Shared store PCs need a fast desktop client, not a browser bookmark.',
    outcomes: ['retailer desktop app', 'Same API contracts as mobile', 'Counter-optimized UX'],
    howItWorks: defaultHowItWorks([
      ['Install on counter PC', 'desktop installer for Windows/macOS.'],
  ...
  'web-apps': seedContent({
    ...
    outcomes: ['Supplier portal for network oversight', 'Warehouse portal for dispatch and fleet', 'Web portals with live updates'],
    howItWorks: defaultHowItWorks([
      ['Supplier login', 'Topology, vetting, treasury dashboards.'],
      ['Warehouse login', 'Dispatch board and fleet map.'],
      ['Live updates', 'live updates-driven refresh.'],
    ]),
    flow: 'appsMatrix',
  ...
  'realtime-coordination': seedContent({
    ...
    outcomes: ['Outbox + live events + WS pipeline', 'Silent refresh on all clients', 'Notification inbox'],
    howItWorks: defaultHowItWorks([
      ['Confirm and save', 'Confirmed save with a reliable change event.'],
      ['Publish events', 'background optimizers invalidate cache.'],
      ['Push to clients', 'Live updates reach each role.'],
    ]),
  ...
  'enterprise-rollout': seedContent({
    ...
    howItWorks: defaultHowItWorks([
      ...
      ['Verify parity', 'Go-live checks per go-live.'],
    ]),
    flow: 'controlPlane',
    flowConfig: { highlightStep: 0 },
  }),
