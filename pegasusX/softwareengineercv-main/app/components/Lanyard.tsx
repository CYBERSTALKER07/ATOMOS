...
function Band({ maxSpeed = 50, minSpeed = 0 }: BandProps) {
  ...
  const nodes = {
    ...
    clip: { geometry: new THREE.CylinderGeometry(0.05, 0.05, 0.1, 8) },
    clamp: { geometry: new THREE.BoxGeometry(0.1, 0.1, 0.05) }
  };

  const materials = {
    metal: new THREE.MeshStandardMaterial({ color: 0xffffff, metalness: 0.9, roughness: 0.1 })
  };

  // Create canvas texture for "THE LAB" text
  const [textTexture] = useState(() => {
    ...
    if (ctx) {
      ...
      ctx.fillText('PEGASUS', canvas.width / 2, canvas.height / 2);
    }
  ...
  return (
    <>
      ...
      <mesh ref={band}>
        {/* @ts-ignore - meshLineGeometry is extended from meshline package */}
        <meshLineGeometry />
        {/* @ts-ignore - meshLineMaterial is extended from meshline package */}
        <meshLineMaterial
          color="white"
          depthTest={false}
