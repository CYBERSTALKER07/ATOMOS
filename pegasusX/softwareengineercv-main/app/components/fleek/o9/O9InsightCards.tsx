...
function ArchitectureFlowDiagram({ isRu, isLight }: { isRu: boolean; isLight: boolean }) {
 ...
 <div className="px-3 py-1.5 rounded-none bg-black border border-white/20 text-white text-[11px] font-semibold tracking-tight whitespace-nowrap">
 ...
 <div className="w-10 h-10 rounded-none bg-white dark:bg-black border border-black dark:border-white flex items-center justify-center dark:">
...
function AgentStackVisual({ isRu, isLight }: { isRu: boolean; isLight: boolean }) {
 ...
 ? 'bg-white border border-black/15 rounded-none px-3.5 py-2.5 flex items-center gap-3 hover:border-black/40 transition-colors group'
 : 'bg-black border border-white/15 rounded-none px-3.5 py-2.5 flex items-center gap-3 hover:border-white/40 transition-colors group';
...
function OutcomeMetricVisual({ isRu }: { isRu: boolean }) {
 ...
 <div className="bg-black border border-white/20 rounded-none px-5 py-4 flex items-center justify-between relative overflow-hidden group hover:border-white/30 transition-all">
 {/* Ambient subtle monochrome glow */}
 <div className="absolute -right-6 -bottom-6 w-24 h-24 bg-white/10 rounded-none pointer-events-none" />

 <span className="text-xs font-mono font-medium tracking-wider uppercase text-white/60">
 {isRu ? 'Валовая маржа' : 'Gross Margin'}
 </span>
 <span className="text-2xl font-bold font-mono tracking-tight text-white drop-">
 +6.2%
 </span>
 </div>
 </div>
...
export default function O9InsightCards({
 ...
 isLight
 ? 'text-zinc-800 bg-white border border-black/10 hover:border-black/25 hover:bg-zinc-50 '
 : 'text-white/80 bg-black border border-white/15 hover:border-white/30 hover:text-white hover:bg-white/5'
 }`}
 >
 ...
 isLight ? 'bg-[#F4F4F6] border-black/5' : 'bg-black border-white/10'
 }`}>
