...
/* =========================================================================
   ...
   ========================================================================= */
function ArchitectureFlowDiagram({ isRu }: { isRu: boolean }) {
  return (
    <div className="relative w-full max-w-[320px] h-[200px] mx-auto flex items-center justify-center select-none">
      ...
      <div className="absolute right-[20px] top-[138px] z-10">
        <div className="px-3 py-1.5 rounded-md bg-black/90 border border-white/20 text-white text-[11px] font-semibold tracking-tight shadow-lg whitespace-nowrap">
          {isRu ? 'Клиентский сервис' : 'Customer Ops'}
        </div>
      </div>
    </div>
  );
...
/* =========================================================================
   ...
   ========================================================================= */
function AgentStackVisual({ isRu }: { isRu: boolean }) {
  return (
    <div className="w-full max-w-[230px] mx-auto flex flex-col gap-2.5 select-none">
      {/* Agent 1: Planning Agent */}
      <div className="bg-[#14141F] border border-white/10 rounded-xl px-3.5 py-2.5 flex items-center gap-3 shadow-[0_4px_16px_rgba(0,0,0,0.4)] hover:border-[#8DDC96]/40 transition-colors group">
      ...
      <div className="bg-[#14141F] border border-white/10 rounded-xl px-3.5 py-2.5 flex items-center gap-3 shadow-[0_4px_16px_rgba(0,0,0,0.4)] hover:border-white/25 transition-colors group">
        <div className="w-7 h-7 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-white/80 shrink-0">
...
/* =========================================================================
   Main Section Component: Dark AI Outcomes & Architecture
   ========================================================================= */
...
export default function O9InsightCards({
  ...
  return (
    <section className="w-full py-12 md:py-16">
        ...
        {showButtons ? (
          <div className="flex items-center gap-3 shrink-0">
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-mono uppercase tracking-wider text-white/80 bg-[#121218] border border-white/15 rounded-lg hover:border-white/30 hover:text-white hover:bg-white/5 transition-all"
            >
              <span>{isRu ? 'Метрики' : 'View Metrics'}</span>
              <ArrowUpRight className="w-3.5 h-3.5 text-[#8DDC96]" />
            </Link>
            <Link
              href="/solutions"
              className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-mono uppercase tracking-wider text-white/80 bg-[#121218] border border-white/15 rounded-lg hover:border-white/30 hover:text-white hover:bg-white/5 transition-all"
      ...
      {/* 3-Column Dark Tactical Bento Grid */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        {/* Column 1: Connected Flow Diagram */}
        <article className="border border-white/10 bg-[#0B0B10] p-6 lg:p-7 rounded-2xl flex flex-col justify-between hover:border-white/20 transition-all duration-300">
          <div className="bg-[#07070B] border border-white/8 rounded-xl p-4 min-h-[220px] flex items-center justify-center relative overflow-hidden">
            <ArchitectureFlowDiagram isRu={isRu} />
          </div>
          <div className="mt-6 flex flex-col flex-1">
            <h3 className="text-lg font-semibold tracking-tight text-white mb-2.5">
              {isRu
