  ...
  return (
    <div className="relative flex h-full min-h-[240px] sm:min-h-[320px] w-full items-center justify-center bg-black px-6 py-10 overflow-hidden">
      <div className="relative w-64 h-64 sm:w-80 sm:h-80 flex items-center justify-center">
        <div className="absolute inset-0 bg-blue-600/20 rounded-none blur-3xl" />
        <Image
 ...
 * Optimized with viewport pausing, adaptive cell grid, and idle throttling
 * to guarantee 60fps scrolling without main-thread jank.
 */
export default function Digit369({
  ...
  const resolvedCell = Math.max(14, cellSize ?? (perf.isMobile ? 16 : 14));
  ...
  useEffect(() => {
    ...
    let cellH = Math.round(resolvedCell * 1.45);
    let cols = 0;
    let rows = 0;
    ...
    // Pre-computed style strings to avoid string allocations per cell per frame
    const styleTier0 = `rgba(${rgb.r},${rgb.g},${rgb.b},0.60)`;
    const styleTier1 = `rgba(${rgb.r},${rgb.g},${rgb.b},0.78)`;
    const styleTier2 = `rgba(${rgb.r},${rgb.g},${rgb.b},0.95)`;
    const bgClear = `rgb(${bg.r},${bg.g},${bg.b})`;
    ...
    const resize = () => {
      ...
      cellW = Math.max(14, resolvedCell);
      cellH = Math.round(cellW * 1.45);
    ...
    const tick = (now: number) => {
      ...
      // 30 FPS when hovered, 12 FPS when idle (preserves 90% CPU)
      const frameInterval = hover ? 33 : 80;
      ...
      // Scramble only a small subset of glyphs in the Pegasus emblem
      const glyphRate = hover ? 0.10 : 0.02;
      const glyphUpdates = Math.max(3, Math.floor(chars.length * glyphRate));
      ...
      if (hover) {
        const bgUpdates = Math.max(3, Math.floor(bgChars.length * 0.03));
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          // Outside Pegasus shape: subtle matrix atmosphere ONLY on hover
          if (tier < 0) {
            if (!hover) continue;
            ...
            const cursorGlow = Math.max(0, 1 - dist * 2.5);

            if ((col + row) % 3 !== 0 && cursorGlow < 0.28) continue;
            const bgAlpha = Math.min(0.18, 0.025 + cursorGlow * 0.25);
            if (bgAlpha < 0.035) continue;
            ctx.fillStyle = `rgba(${rgb.r},${rgb.g},${rgb.b},${bgAlpha.toFixed(2)})`;
          ...
          if (hover) {
            ...
            if (cursorGlow > 0.35) {
              const r = Math.min(255, Math.floor(rgb.r * 0.6 + 59 * 0.4));
              const g = Math.min(255, Math.floor(rgb.g * 0.6 + 130 * 0.4));
              const b = 255;
              const alpha = Math.min(1, 0.8 + cursorGlow * 0.2);
              ctx.fillStyle = `rgba(${r},${g},${b},${alpha.toFixed(2)})`;
            } else {
              ctx.fillStyle = tier === 2 ? styleTier2 : tier === 1 ? styleTier1 : styleTier0;
            }
          } else {
            ctx.fillStyle = tier === 2 ? styleTier2 : tier === 1 ? styleTier1 : styleTier0;
          }
    ...
    // Pause animation when scrolled out of view to eliminate scroll lag
  ...
  return (
    <div
      ...
      className="relative flex h-full min-h-[300px] sm:min-h-[380px] md:min-h-[440px] lg:min-h-full w-full items-center justify-center bg-black overflow-hidden select-none cursor-crosshair group contain-paint"
      style={{
        backgroundColor,
      }}
    >
      {/* Ambient background glow centered on the Pegasus emblem */}
      <div className="absolute inset-0 pointer-events-none opacity-30 bg-[radial-gradient(circle_at_center,_rgba(37,99,235,0.18)_0%,_transparent_75%)]" />
