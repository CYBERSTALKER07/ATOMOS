...
// Mobile-optimized default props
const getMobileDefaults = (isMobile: boolean) => ({
  wispDensity: isMobile ? 0.5 : 1,
  dpr: isMobile ? 1 : undefined,
  fogIntensity: isMobile ? 0.2 : 0.45,
  wispSpeed: isMobile ? 8.0 : 15.0,
  wispIntensity: isMobile ? 2.5 : 5.0,
});
...
export const LaserFlow: React.FC<Props> = (props) => {
  ...
  const mobileDefaults = getMobileDefaults(isMobile);
  const {
    className,
    style,
    wispDensity = mobileDefaults.wispDensity,
    dpr = mobileDefaults.dpr,
    mouseSmoothTime = 0.0,
    ...
    fogIntensity = mobileDefaults.fogIntensity,
    fogScale = 0.3,
    wispSpeed = mobileDefaults.wispSpeed,
    wispIntensity = mobileDefaults.wispIntensity,
    flowStrength = 0.25,
    decay = 1.1,
    falloffStart = 1.2,
    fogFallSpeed = 0.6,
  ...
  useEffect(() => {
    // Skip heavy rendering on very old devices
    const isLowEnd = /iPhone [5-8]|iPad2|iPad3|iPad4|iPad Air(?! [2-9])|iPod|Android [4-5]/i.test(navigator.userAgent);
    if (isLowEnd && isMobile) {
      console.log('LaserFlow: Skipping on low-end device');
      return;
    }
    ...
    // Mobile: Lower DPR significantly
