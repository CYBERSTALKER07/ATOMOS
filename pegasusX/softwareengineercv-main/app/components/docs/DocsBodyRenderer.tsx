...
function CodeBlock({
  ...
  return (
    <div className="my-4 rounded-none overflow-hidden border border-white/10 bg-black">
      {/* Code Header Bar */}
      <div className="flex items-center justify-between px-4 py-2 bg-zinc-950 border-b border-white/10 text-xs font-mono text-zinc-400">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={handleCopy}
            className="flex items-center space-x-1 p-1 rounded-none hover:bg-zinc-900 text-zinc-400 hover:text-white transition-colors"
            title="Copy snippet"
          >
            {copied ? (
              <>
        ...
        <section className="p-6 rounded-none bg-black border border-white/10 space-y-4">
          ...
          <div className="space-y-2.5">
            {article.architectureFlow.steps.map((step, idx) => (
              <div
                key={idx}
                className="flex items-start space-x-3 p-3 rounded-none bg-zinc-950 border border-white/10 text-xs"
              >
          ...
          <div className="space-y-6">
            {article.steps.map((step) => (
              <div key={step.stepNumber} className="relative pl-8 pb-4 border-l border-white/10">
                ...
                <div className="absolute -left-3.5 top-0 w-7 h-7 rounded-none bg-black border border-white text-white font-mono text-xs font-bold flex items-center justify-center shadow-[0_0_10px_rgba(255,255,255,0.15)]">
                  {step.stepNumber}
                </div>
