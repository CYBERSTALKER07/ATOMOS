...
export default function O9DetailLayout({ topic, showFleetShowcase }: O9DetailLayoutProps) {
  ...
  const capabilities = (content.capabilities ?? []).map((cap, i) => ({
    title: cap.title,
  ...
  return (
    <FleekPageShell activeHref={`/${categoryId}`}>
      <O9FleekPageLayout
        ...
        title={isWarehouse ? 'Minimize costs. Transport goods' : content.title}
        ...
        proofItems={content.proofItems ?? DEFAULT_PROOF}
        hubId={categoryId}
        differentiators={content.differentiators ?? []}
        differentiatorsTitle={`Why leaders choose Pegasus ${content.title.toLowerCase()}`}
        outcomes={content.outcomes}
        capabilities={capabilities}
        capabilitiesTitle="What this solution enables"
