...
Last updated: 2026-06-05. Canonical reference: `pegasus/`. Delivery tree: `pegasusX/`.

## Summary

| Role | pegasusX clients | Backend routes | Production v1 capability | UI parity (vs Pegasus) | E2E (SSMR) |
|------|------------------|----------------|--------------------------|------------------------|------------|
| SUPPLIER | supplier-portal, native iOS/Android | supplierroutes | Portal: full ops spine; native: P1-01 ops panels on More hub (exceptions, shop-closed, negotiations, manifests, dispatch preview, activity, fleet orders, ledger, replenishment) | Partial — portal 26+ routes vs Pegasus ~59; native ops panels on Android+iOS More hub | Full SSMR e2e incl. payment + factory |
...
| DRIVER | Android, iOS | driverroutes, orderroutes, telemetryroutes | Full delivery edges + reorder (PX12-B); maps + WS (PX12-F) | Live-ops + maps | Telemetry; shop-closed; negotiation; driver edges E2E |
| WAREHOUSE | portal, Android, iOS | warehouseroutes | Ops dashboard, dispatch lock, supply requests; P1-03 ops API (transfers, replenishment insights, financials, order mutations) | Portal order detail + mutation panel; native order detail mutations (Android+iOS); transfer action panel on More hub (Android+iOS); `warehouse-ops.ts` helper | Dispatch lock + order mutation + transfer actions SSMR markers |
...
| `/fleet`, `/supplier/fleet` | `/(portal)/fleet` | GET/POST `/v1/supplier/fleet/*` | Wired via org-fleet |
