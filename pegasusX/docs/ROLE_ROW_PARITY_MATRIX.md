...
| PAYLOAD | terminal, iOS, Android | payloaderroutes, platformroutes | Manifest lifecycle + reassign + device-token (PX12-K); canonical `/v1/payloader/manifests/*` on all three clients | Lifecycle APIs; unified `manifest.Wire`; seal-completed idempotency aligned across terminal + native | SSMR payload + lifecycle + reassign + driver gate + payloader device-token sub-markers |
