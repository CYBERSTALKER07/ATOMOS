...
| RETAILER | desktop, iOS, Android | retailerroutes, orderroutes, catalogroutes | Order lifecycle (preorder confirm/edit, request-cancel), setup wizard, insights dismiss, catalog/search, tracking + dock, unified checkout | **Wired** — desktop richest; mobile Deliveries hub; pegasus multi-tenant extras out of scope | Register, order create, tracking, `PX_E2E_CATALOG_OK`, retailer SSMR markers |
...
| WAREHOUSE | portal, Android, iOS | warehouseroutes | Ops dashboard + dispatch live fleet map; ops settings, per-SKU inventory policy, enriched supply create | **Wired** — portal + native ops settings, inventory policy, supply create depth (forecast/manual/delivery date); native notifications inbox; pegasus portal-only setup/profile/search handoffs intentional | Dispatch lock + order mutation + transfer actions SSMR markers |
