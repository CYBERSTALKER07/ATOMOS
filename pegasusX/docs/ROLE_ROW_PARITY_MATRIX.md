...
| `/fleet`, `/supplier/fleet` | `/(portal)/fleet` | `FleetScreen` + `FleetLiveMapScreen` | `GET/POST /v1/supplier/fleet/*`, `GET /v1/supplier/fleet/live-map`, `GET/POST /v1/supplier/control-tower/zone-overrides` | **Driver** telemetry; **Warehouse** dispatch freeze | Wired — portal command surface + native read-only zone overrides (PX90-B5) |
