...
| 8 | **Seasonality** | Two templates with `Multiplier` ×1.35 / ×1.15 + Spanner overrides | **Wired:** override `Multiplier` persisted (no read-time 1.2 stub); shared `seasonalcore` builtins for planning + replenishment; Spanner override reader on suggested qty; YoY estimate drafts behind `FORECAST_SEASONAL_ESTIMATE_ENABLED`. Residual: full HW annual library / weather/POS feeds. | `seasonalcore/`; `planning/seasonal_templates.go`; `replenishment/seasonal.go`; marker `PX_E2E_SEASONAL_OVERRIDE_OK` |
...
| 12 | **Cold chain** | `RequiresColdChain`, `StorageTempMinC/MaxC`, `TEMPERATURE_BREACH`, WMS readings | **Partial→Wired (2026-08-06):** product/order cold+hazmat → OR-Tools (§8.5); WMS ingest + quarantine (`WMS_COLD_CHAIN_ENABLED`); **product band hydrate** + same-txn **TEMPERATURE_BREACH** auto-raise with party IDs for inbox. **Residual:** Bluetooth sensor fleet / cumulative minutes outside band. | optimizer contract; `docs/WMS_COLD_CHAIN.md`; `stocklots/coldchain.go` |
| 13 | **Multi-currency** | **19** `Currency` fields in `schema/spanner.ddl` (orders/payments/ledger threaded) | **Partial→Wired (Wave 1+2+, 2026-08-07):** `FxRates` + `ConvertMinor`; payment mismatch gate; billing GMV ConvertMinor → operating; settlement authority `operating_currency_total_minor`; admin/supplier FX list + portal `/settings/fx-rates`; flag-gated `ORDER_CURRENCY_PICKER_*` + `GET /v1/order/currencies` + desktop/Android/iOS picker. **Residual:** multi-currency AR aging ledger, Airwallex live FX. | `fxrates/`; `docs/FX_RATES.md`; markers `PX_E2E_FX_*`, `PX_E2E_ORDER_CURRENCY_PICKER_*` |
| 14 | **i18n** | ~4190 keys × en/ru/uz (desktop + `mobile_*`), `{name}` placeholders → native `%n$s`/`%@` | **Desktop: Wired (draft)**. **Mobile: Wired (draft) + interpolations (2026-08-06)** — Compose `stringResource(id, args)` + iOS `L10n.format`; ~328/369 format call sites. Residual: ~45 pure value displays; ~37 complex embedded-copy/plural strings need ICU plurals or split views; draft linguistic QA. **Not certified Done.** | `packages/i18n/*`; `*-mobile-interpolations.mjs` |
...
- **~~Production Tauri configs ship the dev updater public key~~ (mitigated 2026-08-06)** — `apply_desktop_updater_pubkey.sh` is fail-closed (no silent `dev.pub` default); Windows `installMode` is `basic` (not silent `passive`). Committed configs may still hold `dev.pub` for local builds; production inject requires `TAURI_UPDATER_PUBKEY` and refuses `dev.pub` unless `ALLOW_DEV_UPDATER_PUBKEY=1`. Residual: rotate committed configs off `dev.pub` once prod key is provisioned in all envs.
- **~~Desktop SQLite cache has no TTL and is never cleared on logout~~ (fixed 2026-08-06)** — `cacheGet` honors `updated_at` + `maxAgeMs` (default 15m); `cacheClearAll` on logout (all four desktop Firebase sign-outs) and retailer org-switch; retailer live-data keys are scoped via `scopedCacheKey(retailerId, url)`.
...
- **No CI compiles the 167k lines of mobile code**, no linter runs anywhere (no `.golangci.yml`, no ESLint step, no SwiftLint/detekt/ktlint config in the repo), no `-race`, and no security scanning of any kind. This is *why* P0-2 and P0-3 are in `HEAD`.
  - **Update (2026-08-06 Gate-0 CI):** root workflows [`pegasusx-ci.yml`](../.github/workflows/pegasusx-ci.yml) + [`pegasusx-native-mobile-build.yml`](../.github/workflows/pegasusx-native-mobile-build.yml) now compile all **12** Android/iOS apps, run backend `-race` on critical packages, `golangci-lint` v2, `govulncheck`, and `gitleaks` over `pegasusX/`. Docs: [`docs/GATE0_CI.md`](docs/GATE0_CI.md). Residual: ESLint/jsx-a11y hard gate; SwiftLint/detekt; govulncheck triage.
...
| CI gates | `.github/workflows/` | **Partial→Wired (2026-08-06):** `pegasusx-ci.yml` + `pegasusx-native-mobile-build.yml` cover backend race/golangci/gitleaks + all 12 native apps. Residual: ESLint `jsx-a11y`, SwiftLint/detekt, harden govulncheck. See [`docs/GATE0_CI.md`](docs/GATE0_CI.md). |
...
**Residual.** Full HW-estimated annual template productization still open; real weather/POS feeds; replenishment burn still uses 7-day mean until §8.2. Theatre #8 override multiplier + shared `seasonalcore` windows + YoY estimate drafts are **wired** (see §2 #8).
...
**Refunds and settlement — WIRED (Enterprise Phase 1 / 2026-08-11).** Order refund initiation (full/partial card) + gateway refund + fiscal corrective path — `order/refunds.go`, `payment.RefundCardPayment`; proved by `TestRefund_*` + GP simulator. Supplier payout bank-file execution — `payout/` (gate step 4). `GET /v1/payment/settlement/authority` remains a reporting view. Warehouse ops treasury no longer returns hardcoded fake invoices (`inv-1`); seed/no-Spanner path returns empty invoices / zero overview (fail-closed honesty, 2026-08-06). Progress: [`docs/session-2026-08-07/PHASE1_MONEY_LAW_PROGRESS.md`](./docs/session-2026-08-07/PHASE1_MONEY_LAW_PROGRESS.md).

**Billing meter** (§2.2) — **schema + event decode WIRED (2026-08-06); fee schedule + monthly AR invoice WIRED (2026-08-11):** Spanner columns match; `BillingTierWorker` reads `amount_minor` / nested `total.amount` from live `ORDER_FINALIZED`; `internal/services/billing/` fee schedule + invoice emission proved by `make phase1-gate` step 5.
...
**PR-6 — WIRED (flag `WMS_COLD_CHAIN_ENABLED`).** `TemperatureReadings` ingest + excursion quarantine + product band hydrate + `TEMPERATURE_BREACH` auto-raise — [`docs/WMS_COLD_CHAIN.md`](./docs/WMS_COLD_CHAIN.md).
...
**Cold chain, made real.** `TemperatureReadings` + quarantine + product `StorageTemp*` hydrate + same-txn `TEMPERATURE_BREACH` auto-raise are **wired** behind `WMS_COLD_CHAIN_ENABLED`. Residual: Bluetooth logger fleet and cumulative minutes-outside-band.
...
**Generic offline queue.** **Wired (driver + kit promotion 2026-08-06):** multi-endpoint flush was already live; now rows carry **capture-time** `capturedLat`/`capturedLng`/`capturedAtMs`, Room migrations replace `.fallbackToDestructiveMigration()` on driver, flush replays stored coords (not live GPS). Warehouse/factory Android use kit `PrefsOfflineQueueStore`; payload Android Room v2. Warehouse scan kit + pick-wave increment path wired (Field Harden #9); residual: payload checklist `toggleItem`.

**Proof of delivery.** **Wired (2026-08-06):** credit leave requires `photo_proof_url` (backend fail-closed) and accepts `signature_url`; shop-closed requires a photo. Driver Android + iOS capture PoD photo + signature pad, persist local JPEG paths in the offline queue, and upload via `credit_proof` tickets on flush. Shop-closed waits for photo before report (no auto-report on enter).

**Background location, iOS.** **Wired (2026-08-06):** `Custom-Info.plist` includes `location` in `UIBackgroundModes`; removed malformed conflicting `INFOPLIST_KEY_UIBackgroundModes`; `pausesLocationUpdatesAutomatically = false` and `activityType = .automotiveNavigation` on the fleet location manager. Residual: register or remove inert `BGTaskScheduler` identifier `com.pegasus.driver.sync`.

**Reconnect and reboot.** **Wired (2026-08-06, driver Android):** telemetry + command WS no longer hard-stop after 10 attempts — backoff holds at 60s forever; `resetAndReconnect()` on network `onAvailable` plus `TelemetrySyncWorker` enqueue; `TelemetryService` uses a replaceable `SupervisorJob` (sticky-safe); locations insert to Room **before** WS send and delete only when connected+send ok; `BOOT_COMPLETED` `BootReceiver` resumes tracking when `telemetry_tracking_active` + auth token. Residual: no server-side telemetry ACK frame (OkHttp enqueue ≠ delivery proof); iOS still has no durable offline telemetry buffer.

**Scanning throughput.** Warehouse pick-wave + barcode kit hardened (EAN-8/13, 300ms debounce, torch, keyboard wedge / DataWedge) — see WMS Field Harden #9 / [`docs/WMS_GATE4_HARDENING.md`](./docs/WMS_GATE4_HARDENING.md). Residual: payload checklist `toggleItem` / serial tracking deferred.

**Push, deep links, localization.** Firebase client JSON/plists are **committed** under each mobile app (2026-08). Still required: `aps-environment` entitlements and `PrivacyInfo.xcprivacy` (App Store), declare `FirebaseMessagingService` where missing, refuse `demo-pegasus`/`demo-key` fallbacks, and owner SHA-1 + real SMS for Phone Auth. Register URL schemes and intent filters — `deepLink` is decoded into DTOs in all 12 apps and then discarded because no app declares a handler. **Localization:** catalogs + interpolations wired (`{name}` → Android/iOS positional formats; `L10n.format` / `stringResource` args). Residual: ICU plurals for embedded yes/no/plural-s copy — see §2 #14.
...
| **Warehouse** | Portal good / iOS 60% / Android 55% | **§8.7 Gate 4 + Field Harden #9** (V2 forbid + barcode kit + pick-wave scan); ViewModels residual |
...
**Gate 5 — Multi-tenancy (10–16 weeks).** §8.10 Phase 1, then 2. Do not start before Gate 3, because a partner API and outbound webhooks designed single-tenant will have to be rebuilt. **Phase 1 program ADR:** [`docs/MULTI_TENANCY_GATE5_PHASE1.md`](./docs/MULTI_TENANCY_GATE5_PHASE1.md) (Accepted; foundations Wired — see §8.10).
...
| `warehouse/ops_portal.go` treasury | ~~Hardcoded fake invoice `inv-1`~~ — removed; empty invoices / zero overview when Spanner seed path is active. |
